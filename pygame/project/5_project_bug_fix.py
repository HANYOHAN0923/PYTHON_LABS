...
while running:
   ...
   #캐릭터 rect정보 업데이트
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                break
        else: #refer: double_loop.py & double_loop.png / 3번째 크기 공이 머리에서 맞았을 때 오류 발생
            continue #안쪽 for문 조건이 안 맞으면, 바깥 for문을 실행
        break # 안쪽 for 문에서 break를 만나면 실행
    ...
    timer = game_font.render(f"Time : {int(total_time - elasped_time)}",True,(255,255,255))
